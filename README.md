...
Credentials are stored in `$XDG_CONFIG_HOME/bb/hosts.yml` (mode 0600). On
Windows, where file modes are not enforced, prefer environment variables.
...
|---|---|---|
| `bb auth login/logout/status/token/setup-git` | `gh auth …` | API tokens; `--bearer` for access tokens |
...
| `bb browse` | `gh browse` | |
| `bb config get/set/list` | `gh config …` | `workspace`, `git_protocol`, `merge_strategy`, `editor`, `pager`, `browser` |
...
- OAuth/device-flow login is not available (Bitbucket has no device flow);
  API tokens are the supported path.
