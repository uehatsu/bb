...
このスキルは、Codex が `bb` を安全かつ効率的に使うための手順書です。
...
2. **非対話で動かす**: Codex のコマンド実行環境は TTY ではない。プロンプトが出るコマンドには明示フラグを付ける。例: `pr create --title/--body`, `pr merge --squash|--merge|--rebase`, `repo delete --yes`, `branch delete --yes`, `repo create --private|--public`。
